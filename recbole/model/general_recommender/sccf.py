...
class MFEncoder(nn.Module):
    ...
    def lightgcn_init(self, norm_adj, n_layers: int=100):
        lightgcn = LGCNEncoder(self.user_num, self.item_num, self.emb_size, norm_adj, n_layers).cuda()
        nn.init.xavier_normal_(lightgcn.user_embedding.weight)
        nn.init.xavier_normal_(lightgcn.item_embedding.weight)
        user_e, item_e = lightgcn.get_all_embeddings()
        self.user_embedding = nn.Embedding.from_pretrained(user_e).requires_grad_()
        self.item_embedding = nn.Embedding.from_pretrained(item_e).requires_grad_()
